...
inline std::unique_ptr<ResponceType> Server::receiveMessageFrom(size_t index)
{
    return receiveMessageFrom<ResponceType>(*this->m_connections[index]);
}
template<typename ResponceType>
std::unique_ptr<ResponceType> Server::receiveMessageFrom(const Connection& connection)
{
    ...
    std::unique_ptr<ResponceType> responce = std::make_unique<ResponceType>(buffer, data_size);
